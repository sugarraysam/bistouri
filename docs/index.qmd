...
graph TD
    subgraph "Kernel Space"
        ...
        SCHED["Scheduler (sched_switch)"] --> BPF_PROG
...
3.  **The Filtering:** We use a Longest Prefix Match (LPM) Trie in BPF for `exec` hooks and a standard Hash Map for active PID filtering. The `TriggerAgent` manages rule-based matching, while the `CaptureOrchestrator` manages the lifecycle of active PIDs.
4.  **The Capture:** The BPF profiler uses dual triggers: a hardware timer (e.g., 19Hz) for on-CPU samples and a scheduler hook for off-CPU samples. It verifies the PID is "hot" in the filter map before capturing.
...
-   **User-space Triggering:** One might ask why we don't trigger the profiler entirely in the kernel. The answer is policy. Defining what constitutes "too much pressure" involves complex configuration, threshold percentages, and multi-tenant metadata that is significantly easier and safer to manage in Rust.
