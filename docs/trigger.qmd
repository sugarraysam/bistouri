...
Additionally, the walk serves as a metadata collection phase. For every matched process, we read `/proc/<pid>/auxv` to resolve its vDSO address range. This information is stored in a shared `VdsoCache` (an LRU cache), allowing the profiler to classify stack frames (marking them as `[vdso]` instead of `[unknown]`) without performing blocking I/O during the high-frequency sampling path.
