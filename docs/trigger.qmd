...
To prevent accumulating leaked file descriptors and inflating active watcher metrics when processes exit or are recycled, each watcher periodically performs an $O(1)$ liveness check using `/proc/<pid>/comm`. Since this virtual file is serviced directly by the kernel, it is extremely fast and can be safely queried from the asynchronous loop to reap dead watchers.
...
The BPF trie is the "fast path." When a new process is `exec`'d, our BPF programs check the trie to see if the process name matches any profiling rules. If it does, it immediately notifies userspace to set up a PSI watcher for that process's cgroup. To keep the interface clean, the BPF map value is a simple `rule_id`, which the userspace agent uses to look up the full configuration (including thresholds and labels).
...
Additionally, the walk serves as a metadata collection phase. For every matched process, we read `/proc/<pid>/auxv` to resolve its vDSO address range and perform ELF section scanning to detect its runtime (e.g. Go). These blocking I/O operations are done entirely on the blocking worker pool (`spawn_blocking`), populating the shared `VdsoCache` (an LRU cache) and `ProcessMatchEvent` so that the high-frequency sampling path never performs blocking disk or memory lookups during execution.

## Hot Reload Without Downtime

Bistouri is designed for long-running production use, meaning we must support configuration changes without restarting the agent. The `TriggerAgent` supports two discovery strategies:

1.  **File Watcher:** Uses `inotify` to watch a YAML file on disk. It includes a 200ms debounce timer to handle "atomic" file swaps. This is critical for Kubernetes environments, where ConfigMap updates often involve a rapid sequence of directory symlink renames that would otherwise trigger multiple redundant reloads.
2.  **Kubernetes Watcher:** Watches a `BistouriConfig` Custom Resource (CR) via the Kubernetes API. This allows for centralized configuration management across a cluster.
...
graph TD
    ...
    subgraph "Userspace - Trigger Agent"
        RING_BUF -->|"Event (rule_id)"| EV_LOOP["Event Loop"]
        PROC_SCAN["Periodic /proc Walk"] -->|"Match"| EV_LOOP

        subgraph "Watchers"
            INOTIFY["inotify (File)"]
            KUBE["kube-rs (CRD)"]
        end

        INOTIFY -->|"Reload"| EV_LOOP
        KUBE -->|"Reload"| EV_LOOP
    ...
    SPAWN -->|"CaptureRequest"| PROFILER["Capture Orchestrator"]
```
