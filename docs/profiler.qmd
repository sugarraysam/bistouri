...
In userspace, these are caught, decoded, and logged with specific context—like suggesting an increase in buffer size if drops are detected. The use of a C union in the BPF side, mapped to a Rust `enum` in the agent, gives us a type-safe way to handle these heterogeneous error events without the overhead of multiple maps.
