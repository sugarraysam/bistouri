...
A `MapHandle` duplicates the underlying file descriptor of a BPF map. This is critical for our architecture because it allows the `TriggerAgent` (which determines which processes to profile) to update the `comm_lpm_trie` independently of the `ProfilerAgent`'s main loop. 
:::
...
1.  **`ERR_GET_STACK`**: If `bpf_get_stack` returns a negative code (e.g., because the stack was paged out).
2.  **`ERR_RINGBUF_RESERVE`**: If the ring buffer is full and we dropped a sample.
