...
# Stop systemd k3s if it is running
if systemctl is-active --quiet k3s 2>/dev/null; then
	echo "Pausing systemd k3s service for isolated E2E environment..."
	sudo systemctl stop k3s
fi

# Custom configs to speed up ConfigMap hot-reload
cat <<EOF | sudo tee /tmp/bistouri-kubelet.config >/dev/null
...
if ! sudo k3s kubectl get node >/dev/null 2>&1; then
	sudo nohup k3s server --disable=traefik --disable=servicelb \
		...
		--kubelet-arg="config=/tmp/bistouri-kubelet.config" \
		>/tmp/k3s.log 2>&1 &
	K3S_STARTED=true
